...
const inp = 'w-full px-3 py-2 text-xs font-mono outline-none bg-[var(--inp-bg)] border-2 border-[var(--border)] text-[var(--text)] transition-all focus:bg-[var(--inp-focus)] focus:text-black focus:shadow-[2px_2px_0px_0px_var(--border)] focus:translate-x-[-1px] focus:translate-y-[-1px]';
...
function Row({ label, value, accent }: { label: string; value: string; accent: string }) {
  return (
    <div className="flex items-center gap-2 px-3 py-2 border-2 border-[var(--border)] bg-[var(--inp-bg)] transition-colors hover:bg-[var(--row-hover)]">
...
function SectionLabel({ icon: Icon, label }: { icon: React.ElementType; label: string }) {
  return (
    <div className="flex items-center gap-1.5 mb-3 bg-black dark:bg-white text-white dark:text-black px-2 py-0.5 border border-black dark:border-white inline-flex max-w-max">
      <Icon size={10} />
...
export default function Page() {
  ...
  return (
    <main className="min-h-screen bg-[var(--bg)] flex items-start md:items-center justify-center p-4 md:p-8 transition-colors duration-300">
      <div className="w-full max-w-3xl border-3 border-[var(--border)] bg-[var(--card)] shadow-[8px_8px_0px_0px_var(--border)] transition-all">

        {/* ── Header ── */}
        <div className="px-4 md:px-6 py-4 border-b-3 border-[var(--border)] bg-yellow-400 dark:bg-yellow-500 text-black flex items-center justify-between gap-2">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-sm font-black text-black tracking-tight uppercase">TM3 Converter</h1>
              <p className="text-[9px] font-extrabold text-black/80 uppercase tracking-widest hidden sm:block">Indonesia · DGN95 / WGS84</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <select value={zone} onChange={e => { setZone(e.target.value); setEditingTm3(false); setEditingWgs(false); setEditingDms(false); }}
                className="appearance-none pl-3 pr-8 py-1.5 bg-white border-2 border-black rounded-none text-xs text-black font-extrabold outline-none hover:bg-zinc-100 transition-all cursor-pointer shadow-[2px_2px_0px_0px_#000]">
                {TM3_ZONES.map(z => <option key={z.label} value={z.label}>Zone {z.label}</option>)}
              </select>
              <ChevronDown size={10} className="absolute right-2 top-1/2 -translate-y-1/2 text-black pointer-events-none font-bold" />
            </div>
            {/* Theme toggle */}
            <button onClick={() => setDark(d => !d)}
              className="w-8 h-8 border-2 border-black bg-white flex items-center justify-center hover:bg-yellow-300 transition-all shadow-[2px_2px_0px_0px_#000] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none">
              {dark ? <Sun size={14} className="text-amber-500 font-bold" /> : <Moon size={14} className="text-blue-600 font-bold" />}
            </button>
          </div>
        </div>

        {/* ── Body: single col mobile, two col desktop ── */}
        <div className="grid grid-cols-1 md:grid-cols-2 md:divide-x-3 divide-[var(--border)]">
          ...
          <div className="px-4 md:px-6 py-6 space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <SectionLabel icon={Globe} label="WGS84 Geografis" />
                <div className="flex border-2 border-[var(--border)] bg-white dark:bg-zinc-800">
                  {(['dd', 'dms'] as const).map(m => (
                    <button key={m} onClick={() => { setEditingWgs(false); setEditingDms(false); setWgsMode(m); }}
                      className={`px-3 py-1 text-[9px] font-black uppercase transition-colors flex-1 ${wgsMode === m ? 'bg-black text-white dark:bg-white dark:text-black' : 'text-[var(--text)] hover:bg-zinc-100 dark:hover:bg-zinc-700'}`}>
                      {m}
                    </button>
                  ))}
                </div>
            ...
            <div className="relative">
              <SectionLabel icon={Link} label="Google Maps Link" />
              <input 
                type="text" 
                value={gmapsLink} 
                ...
                className={inp + " pr-8"} 
              />
              <div className="absolute right-2.5 top-9">
                {gmapsStatus === 'idle' && <Link size={14} className="text-[var(--text)]" />}
                {gmapsStatus === 'processing' && <Loader2 size={14} className="text-yellow-500 animate-spin" />}
                {gmapsStatus === 'success' && <Check size={14} className="text-emerald-500 font-bold" />}
                {gmapsStatus === 'error' && <AlertCircle size={14} className="text-red-500 font-bold" />}
              </div>
            </div>
            ...
            {/* TM3 */}
            <div>
              <SectionLabel icon={MapPin} label="TM3 — Easting / Northing" />
              <div className="grid grid-cols-2 gap-2.5">
            ...
            {/* Batch CSV */}
            <div>
              <SectionLabel icon={UploadCloud} label="Batch CSV" />
              <label htmlFor="csv-file"
                className="flex items-center gap-4 px-4 py-3 border-2 border-dashed border-[var(--border)] bg-[var(--inp-bg)] cursor-pointer hover:bg-yellow-400/10 dark:hover:bg-yellow-400/5 transition-all shadow-[3px_3px_0px_0px_var(--border)] active:translate-x-[1.5px] active:translate-y-[1.5px] active:shadow-none group">
                <div className="w-8 h-8 bg-white dark:bg-zinc-800 border-2 border-black dark:border-white flex items-center justify-center shrink-0">
                  <UploadCloud size={14} className="text-black dark:text-white" />
                </div>
                <div>
              ...
              {csvMsg && <p className={`mt-2 text-[10px] font-black ${csvMsg.startsWith('Gagal') ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'}`}>{csvMsg}</p>}
            </div>
          </div>

          {/* RIGHT — Results */}
          <div className="px-4 md:px-6 py-6 border-t-3 border-[var(--border)] md:border-t-0 bg-zinc-50 dark:bg-zinc-900/40 flex flex-col">
            <p className="text-[10px] font-black text-[var(--text)] uppercase tracking-widest mb-4 bg-black dark:bg-white text-white dark:text-black inline-block px-2 py-0.5 max-w-max border border-black dark:border-white">Hasil Konversi</p>

            <div className="space-y-2 flex-1">
              <p className="text-[8px] font-black text-[var(--text)] uppercase tracking-wider px-1">WGS84 Geografis</p>
              <Row label="Latitude"  value={coords.lat.toFixed(6)} accent="text-yellow-600 dark:text-yellow-500 font-extrabold" />
              <Row label="Longitude" value={coords.lon.toFixed(6)} accent="text-yellow-600 dark:text-yellow-500 font-extrabold" />
              <Row label="DMS Lat"   value={dmsLat} accent="text-yellow-600 dark:text-yellow-500 font-extrabold" />
              <Row label="DMS Lon"   value={dmsLon} accent="text-yellow-600 dark:text-yellow-500 font-extrabold" />
              <Row label="DD Pair"   value={`${coords.lat.toFixed(6)}, ${coords.lon.toFixed(6)}`} accent="text-yellow-600 dark:text-yellow-500 font-extrabold" />

              <div className="my-3 h-0.5 bg-[var(--border)]" />

              <p className="text-[8px] font-black text-[var(--text)] uppercase tracking-wider px-1">
                TM3 Zone {zone} · CM {curZone?.meridian}°
              </p>
              <Row label="Easting X"  value={tm3.x} accent="text-yellow-600 dark:text-yellow-500 font-extrabold" />
              <Row label="Northing Y" value={tm3.y} accent="text-yellow-600 dark:text-yellow-500 font-extrabold" />
              <Row label="X, Y"       value={`${tm3.x}, ${tm3.y}`} accent="text-yellow-600 dark:text-yellow-500 font-extrabold" />
            </div>

            <p className="text-[9px] font-extrabold text-[var(--faint)] mt-4">© 2026 Coordinate Converter</p>
          </div>
        </div>
      </div>
    </main>
